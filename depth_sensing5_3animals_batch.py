...
#white
    x_pixel_M3head = csv_data[19][:].tolist()
    y_pixel_M3head = csv_data[20][:].tolist()
    
    return(x_pixel_M1head, y_pixel_M1head, x_pixel_M2head, y_pixel_M2head, x_pixel_M3head, y_pixel_M3head)
        

def extract_xyz(x_pixel_M1, y_pixel_M1, x_pixel_M2, y_pixel_M2, x_pixel_M3, y_pixel_M3, outputfilename, svofilename):
    x_world_M1 = []
    y_world_M1 = []
    z_world_M1 = []
    ...
    try:    
        while i < len(x_pixel_M1):
            ...
            if zed.grab(runtime_parameters) == sl.ERROR_CODE.SUCCESS:
                ...
                err_M3, point_cloud_value_M3 = point_cloud.get_value(x_pixel_M3[i], y_pixel_M3[i])
                ...
                x_world_M3.append(point_cloud_value_M3[0] + lens2cage_edge_x_mm)
                y_world_M3.append(-point_cloud_value_M3[1] + lens2cage_edge_y_mm)
        ...
        rows = zip(x_world_M1, y_world_M1, z_world_M1, x_world_M2, y_world_M2, z_world_M2, x_world_M3, y_world_M3, z_world_M3)
...
def main(xy_filepath, svofilename, outputfilename):
    #xy_filepath = sys.argv[1]
    #outputfilename = sys.argv[3]    
    #svofilename = sys.argv[2]
    print(xy_filepath)
    print(outputfilename)
    print(svofilename)
    x_pixel_M1head, y_pixel_M1head, x_pixel_M2head, y_pixel_M2head, x_pixel_M3head, y_pixel_M3head = read_xy_csv(xy_filepath)
    extract_xyz(x_pixel_M1head, y_pixel_M1head, x_pixel_M2head, y_pixel_M2head, x_pixel_M3head, y_pixel_M3head, outputfilename, svofilename)
