...
import depth_sensing5_3animals_batch
import os, re
    ...
    for f in file:
        ...
        if re.match('.*000.csv$', f):
            ...
            if os.path.exists(svofilename):
                ...
                outputfilename = basedir + "\\mp4s\\" + rec_date + "_RWC.csv"
                #outputfilename_arr.append(outputfilename)
                if not os.path.exists(outputfilename):   
                    depth_sensing5_3animals_batch.main(xy_filepath, svofilename, outputfilename)
